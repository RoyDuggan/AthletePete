...
export type AvailableLap = {
    ...
    /** Multi-session fields: globally-unique id + which file the lap is from. */
    lapId?: string;
    sessionId?: string;
    sessionNumber?: number;
    /** Session-prefixed display label, e.g. "S1 Lap 3". */
    label?: string;
};

/** One uploaded file in a session group, numbered by upload order. */
...
export type AdvisoryData = {
    ...
    /** Token (subject lap's upload filename) used to re-run analysis. */
    sessionId?: string;

    /** All uploaded files in the group (numbered by upload order). */
    sessions?: SessionInfo[];

    /** All comparable laps across every session, so the user can pick two. */
    ...
    /** Which file each compared lap came from (cross-session aware). */
    subjectSessionId?: string | null;
    referenceSessionId?: string | null;

    /** Session-prefixed labels for the current comparison, e.g. "S1 Lap 3". */
...
/** A saved, reusable custom zone map (named library entry). */
export type ZoneMap = {
    id: string;
    name: string;
    boundaries: number[];
